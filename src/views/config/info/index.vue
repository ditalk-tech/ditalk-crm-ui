    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          ...
          <el-col :span="1.5">
            <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()" v-hasPermi="['config:info:remove']"
              >删除</el-button
            >
          </el-col>
          <el-col :span="1.5">
...
const data = reactive<PageData<InfoForm, InfoQuery>>({
  ...
  rules: {
    ...
    name: [{ required: true, message: '配置项名称不能为空', trigger: 'blur' }],
    code: [{ required: true, message: '配置项键不能为空', trigger: 'blur' }],
    value: [{ required: true, message: '配置项JSON值不能为空', trigger: 'blur' }]
  }
});
