<template>
  <div class="p-2">
    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['event:moment:add']">新增</el-button>
          </el-col>
          <el-col :span="1.5">
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width" fixed="right">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['event:moment:edit']"></el-button>
            </el-tooltip>
            <el-tooltip content="删除" placement="top">
              <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['event:moment:remove']"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog :title="dialog.title" v-model="dialog.visible" width="960px" append-to-body>
      <el-form ref="momentFormRef" :model="form" :rules="rules" label-width="120px">
        <el-form-item label="活动ID" prop="eventId">
...
<script setup name="Moment" lang="ts">
import { listMoment, getMoment, delMoment, addMoment, updateMoment } from '@/api/event/moment';
import { MomentVO, MomentQuery, MomentForm } from '@/api/event/moment/types';
...
const momentFormRef = ref<ElFormInstance>();
...
const getList = async () => {
  ...
  const res = await listMoment(queryParams.value);
...
/** 提交按钮 */
const submitForm = () => {
  momentFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
