        ...
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            ...
            <el-form-item label="指派部门" prop="assignedDept">
              <el-input v-model="queryParams.assignedDept" placeholder="请输入指派部门" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            <el-form-item label="状态" prop="state">
              <el-select v-model="queryParams.state" placeholder="请选择状态" clearable>
                <el-option v-for="dict in ditalk_contract_state" :key="dict.value" :label="dict.label" :value="dict.value" />
              </el-select>
            </el-form-item>
            <el-form-item>
    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['contract:info:add']">新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['contract:info:edit']"
              >修改</el-button
            >
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()" v-hasPermi="['contract:info:remove']"
              >删除</el-button
            >
          </el-col>
          <el-col :span="1.5">
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['contract:info:export']">导出</el-button>
          </el-col>
      ...
      <el-table v-loading="loading" border :data="infoList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="状态" align="center" prop="state">
          <template #default="scope">
            <dict-tag :options="ditalk_contract_state" :value="scope.row.state" />
          </template>
        </el-table-column>
    ...
    <el-dialog :title="dialog.title" v-model="dialog.visible" width="960px" append-to-body>
      <el-form ref="infoFormRef" :model="form" :rules="rules" label-width="120px">
        ...
        <el-form-item label="税费金额" prop="taxAmount">
          <el-input-number v-model="form.taxAmount" placeholder="请输入税费金额" :min="0" :precision="2" />
        </el-form-item>
        ...
        <el-form-item label="状态" prop="state">
          <el-radio-group v-model="form.state">
            <el-radio v-for="dict in ditalk_contract_state" :key="dict.value" :value="dict.value">{{ dict.label }}</el-radio>
          </el-radio-group>
        </el-form-item>
      </el-form>
      <template #footer>
...
const dateRangeSignDate = ref<[DateModelType, DateModelType]>(['', '']);
...
const getList = async () => {
  ...
  proxy?.addDateRange(queryParams.value, dateRangeSignDate.value, 'SignDate');
  ...
  infoFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      ...
      if (form.value.id) {
        ...
        await addInfo(copyForm).finally(() => (buttonLoading.value = false));
      }
...
const handleDelete = async (row?: InfoVO) => {
  ...
  await proxy?.$modal.confirm('是否确认删除合同信息编号为"' + _ids + '"的数据项？').finally(() => (loading.value = false));
  await delInfo(_ids);
