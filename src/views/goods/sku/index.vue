    ...
    <el-dialog :title="dialog.title" v-model="dialog.visible" width="960px" append-to-body>
      <el-form ref="skuFormRef" :model="form" :rules="rules" label-width="120px">
        ...
        <el-form-item label="锁定库存" prop="reservedStock">
          <el-input v-model="form.reservedStock" placeholder="请输入锁定库存" />
        </el-form-item>
        <el-form-item label="已占库存" prop="allocatedStock">
          <el-input v-model="form.allocatedStock" placeholder="请输入已占库存" />
        </el-form-item>
        <el-form-item label="不可用库存" prop="unavailableStock">
          <el-input v-model="form.unavailableStock" placeholder="请输入不可用库存" />
        </el-form-item>
...
const defaultGoodsId = ref<string>();
...
const data = reactive<PageData<SkuForm, SkuQuery>>({
  ...
  rules: {
    ...
    skuSn: [{ required: true, message: 'SKU编码不能为空', trigger: 'blur' }],
...
const resetQuery = () => {
  ...
  handleQuery();
  queryParams.value.goodsId = defaultGoodsId.value;
};
