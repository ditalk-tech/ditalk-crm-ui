        ...
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            ...
            <el-form-item label="报价单ID" prop="quotationId">
              <el-input v-model="queryParams.quotationId" placeholder="请输入报价单ID" clearable @keyup.enter="handleQuery" />
            </el-form-item>
    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['opportunity:quotationItem:add']">新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['opportunity:quotationItem:edit']"
              >修改</el-button
            >
          </el-col>
          <el-col :span="1.5">
            <el-button
              ...
              :disabled="multiple"
              @click="handleDelete()"
              v-hasPermi="['opportunity:quotationItem:remove']"
              >删除</el-button
            >
          </el-col>
          <el-col :span="1.5">
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['opportunity:quotationItem:export']">导出</el-button>
          </el-col>
          <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
        </el-row>
      </template>

      <el-table v-loading="loading" border :data="quotationItemList" @selection-change="handleSelectionChange">
            ...
            <el-button-group>
              <el-tooltip content="修改" placement="top">
                <el-button
                  ...
                  v-hasPermi="['opportunity:quotationItem:edit']"
                ></el-button>
              </el-tooltip>
    ...
    <el-dialog :title="dialog.title" v-model="dialog.visible" width="960px" append-to-body>
      <el-form ref="quotationItemFormRef" :model="form" :rules="rules" label-width="120px">
        ...
        <el-form-item label="图片" prop="mainPic">
          <image-upload v-model="form.mainPic" />
        </el-form-item>
...
const initFormData: QuotationItemForm = {
  id: undefined,
  version: undefined,
...
const reset = () => {
  ...
  quotationItemFormRef.value?.resetFields();
};
...
/** 多选框选中数据 */
const handleSelectionChange = (selection: QuotationItemVO[]) => {
...
/** 修改按钮操作 */
const handleUpdate = async (row?: QuotationItemVO) => {
  reset();
...
/** 删除按钮操作 */
const handleDelete = async (row?: QuotationItemVO) => {
  const _ids = row?.id || ids.value;
  await proxy?.$modal.confirm('是否确认删除商机报价单明细编号为"' + _ids + '"的数据项？').finally(() => (loading.value = false));
  await delQuotationItem(_ids);
...
const handleExport = () => {
  proxy?.download(
    'opportunity/quotationItem/export',
    {
      ...queryParams.value
    },
